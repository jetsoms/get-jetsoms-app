...
while getopts hvdfrnawi:x:-: OPT; do
    ...
    case "$OPT" in
    h | help)
        echo -e "App installer $VERSION
(source code: https://github.com/jetsoms/get-jetsoms-app/blob/main/static/coolify/install.sh)\n
Usage: install.sh [options...] 
    ...
    -r, --restart               Only restart the app. Nothing more.\n

    -n, --do-not-track          Opt-out of telemetry. You can set export DO_NOT_TRACK=1 in advance.\n
    -a, --auto-update           Warning: TESTING PHASE, DO NOT USE IT YET! Enable auto update feature of the app. \n
    ...
    -x, --force-version         Force installation of a specific version of the app."
        exit 1
        ;;
    d | debug)
        DEBUG=1
...
if [ $FORCE -eq 1 ]; then
    echo -e "Installing the app with force option!"
else
    echo -e "Welcome to our installer!"
    echo -e "This script will install all requirements to run the app."
    echo -e "(Source code of this script: https://github.com/jetsoms/get-jetsoms-app/blob/main/static/coolify/install.sh)\n"
    echo "-------------"
    echo -e "TELEMETRY:"
    echo -e "1. The script generates a random UUID for your installation to show the number of installed instances on the landing page. Nothing else."
...
if [ "$(sudo docker info --format '{{.Swarm.ControlAvailable}}')" = "true" ]; then
    echo "This app does not support Docker Swarm yet. Please use a non-swarm compatible version of Docker."
    exit 1
fi
...
if [ ! -x ~/.docker/cli-plugins/docker-compose ]; then
    ...
    if [ ARCH == 'arm64' ]; then
        sudo curl --silent -SL https://cdn.jetsoms.co.uk/bin/linux/arm64/docker-compose-linux-2.6.1 -o ~/.docker/cli-plugins/docker-compose
        sudo chmod +x ~/.docker/cli-plugins/docker-compose
    fi
    if [ ARCH == 'aarch64' ]; then
        sudo curl --silent -SL https://cdn.jetsoms.co.uk/bin/linux/aarch64/docker-compose-linux-2.6.1 -o ~/.docker/cli-plugins/docker-compose
        sudo chmod +x ~/.docker/cli-plugins/docker-compose
    fi
    if [ ARCH == 'amd64' ]; then
        sudo curl --silent -SL https://cdn.jetsoms.co.uk/bin/linux/amd64/docker-compose-linux-2.6.1 -o ~/.docker/cli-plugins/docker-compose
        sudo chmod +x ~/.docker/cli-plugins/docker-compose
    fi
fi
if [ $FORCE -eq 1 ]; then
    ...
    if [ -f "$COOLIFY_CONF_FOUND" ]; then
        while true; do
            read -p "App configuration found (${COOLIFY_CONF_FOUND}). I will overwrite it, okay?  [Yy/Nn] " yn
            case $yn in
            [Yy]*)
                saveCoolifyConfiguration
                break
...
if [ $FORCE -ne 1 ]; then
    echo "Installing this app..."
fi

set +e
echo "Pulling latest image (${VERSION}) from ghcr.io."
TAGVERSION=${VERSION}
LATESTVERSION="latest"

IMAGE=ghcr.io/jetsoms/host-jetsoms-app:${TAGVERSION}
if ! docker pull -q $IMAGE >/dev/null 2>&1; then
    IMAGE=ghcr.io/jetsoms/host-jetsoms-app:${LATESTVERSION}
    if ! docker pull -q $IMAGE >/dev/null 2>&1; then
        IMAGE=jetsoms/host-jetsoms-app:${TAGVERSION}
        if ! docker pull -q $IMAGE >/dev/null 2>&1; then
            IMAGE=jetsoms/host-jetsoms-app:${LATESTVERSION}
            if ! docker pull -q $IMAGE >/dev/null 2>&1; then
                echo "Cannot find any images. Please check your internet connection. Exiting."
                exit 1
            else
